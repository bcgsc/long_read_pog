...
ggplot(meth_data_grouped, 
  ...
  scale_fill_manual(values = c("Tumour"="#8A7EC7","WGBS"="#69BDC3"))+
  scale_color_manual(values = c("Tumour"="#8A7EC7","WGBS"="#69BDC3"))+
...
just_wgbs<- meth_data%>%filter(Type=="WGBS")
just_wgbs$GainIDH_Or_LOFTET<- "WGBS"
...
meth_data_mutation$GainIDH_Or_LOFTET<- factor(meth_data_mutation$GainIDH_Or_LOFTET,
                                              levels = c("No","Yes","WGBS"))
comp<- list(c("No","Yes"),c("Yes","WGBS"),c("No","WGBS"))
...
for (i in input_files){
  ...
  for (t_type in types){
    ...
    readymat$stdev <- apply(filtered_readymat, 1, sd, na.rm=FALSE)
    names(readymat)[length(names(readymat))]<- t_type
  }
  if (dim(readymat)[1] > 2000) {
    readymat$row_stdev <- apply(readymat[,158:168], 1, sd, na.rm=FALSE)
    readymat<- readymat[order(-readymat$row_stdev),]
    readymat<- readymat[1:2000,1:157]
  }
...
plot_tsne= tsne(readymat_merge %>% mutate_at(1:157, as.numeric)
                ...
                perplex = 15,textlabelsize = 1.5,seed=100,
                controlscale=TRUE,scale=3,dotsize = 4)
print(plot_tsne)
```

# t-SNE based on biopsy site
```{r tSNE_Biopsy_Sites, warning=F, message=F, fig.width=9, fig.height=5,echo=F}
readymat_merge<- data.frame()
...
for (i in input_files){
  ...
  if (dim(readymat)[1] > 2000) {
    ...
    readymat<- readymat[order(-readymat$row_stdev),]
    readymat<- readymat[1:2000,1:182]
  }
  readymat_merge<- rbind(readymat_merge,readymat)
}
plot_tsne= tsne(readymat_merge %>% mutate_at(1:182, as.numeric),
                ...
                seed=100,controlscale=TRUE,scale=3,dotsize = 3)
print(plot_tsne)
```
...
p1<- ggplot(filter(readymat,variable %like% "^Frac"), 
            aes(x=gsub("Frac_","",variable), 
                y=value))+
  geom_boxplot(notch=FALSE,outlier.shape = NA, position = position_dodge(0.8))+
  stat_boxplot(aes(ymin = after_stat(ymin), ymax = after_stat(ymax)), geom="errorbar", width=.25,color="black") + 
  stat_boxplot(aes(ymin = after_stat(middle), ymax = after_stat(middle)), geom="errorbar", linewidth=1, width=.25,color="black")  +
...
mat$Type<- factor(mat$Type, levels = c("Tumour","Blood","WGBS"))
anno_Tumour_type = c("#7C3A88","#8A7EC7","#69BDC3","#0B8892","#406070")
names(anno_Tumour_type)<- c("Allele1_Tumour","Allele2_Tumour",
                            "Allele1_Blood","Allele2_Blood","WGBS_WGBS")
ggplot(filter(mat, Position>= 43101000 & Position<=43108500 & Gene=="RET"),  
                  aes(x=Position, 
                      y=value,color=paste(Allele,Type,sep="_")))+
...
mat$Category<- factor(mat$Category, levels = c("IntraMethylated","IntraUnmethylated",
                                               "IntraAllelicMethylated","GTEx"))
mat_ret<-filter(mat,value>1 & Gene=="RET")
p1<- ggplot(mat_ret, aes(x=Type,y=log2(value), fill=Type,color=Type))+
  scale_fill_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  scale_color_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  geom_boxplot(notch=FALSE,outlier.shape = NA, position = position_dodge(0.8))+
  stat_boxplot(aes(ymin = after_stat(ymin), ymax = after_stat(ymax)), geom="errorbar", width=.25,color="black") + 
  stat_boxplot(aes(ymin = after_stat(middle), ymax = after_stat(middle)), geom="errorbar", linewidth=1, width=.25,color="black")  + 
...
my_comparisons <- list( c("IntraMethylated", "GTEx"), c("IntraUnmethylated", "GTEx"), 
                        c("IntraMethylated","IntraUnmethylated") )
p2<- ggplot(filter(mat_ret, Category%in%c("IntraMethylated","IntraUnmethylated","GTEx")), 
            aes(x=Category,y=log2(value), fill=Type,color=Type))+
  scale_fill_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  scale_color_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  geom_boxplot(notch=FALSE,outlier.shape = NA, position = position_dodge(0.8))+
  stat_boxplot(aes(ymin = after_stat(ymin), ymax = after_stat(ymax)), geom="errorbar", width=.25,color="black") + 
...
p3<- ggplot(filter(mat_ret, Category%in%c("IntraAllelicMethylated","GTEx")), 
            aes(x=Category,y=log2(value), fill=Type,color=Type))+
  scale_fill_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  scale_color_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  geom_boxplot(notch=FALSE,outlier.shape = NA, position = position_dodge(0.8))+
...
p1<- ggplot(mat_cdk, aes(x=Type,y=log2(value), fill=Type,color=Type))+
  scale_fill_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  scale_color_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  geom_boxplot(notch=FALSE,outlier.shape = NA, position = position_dodge(0.8))+
  stat_boxplot(aes(ymin = after_stat(ymin), ymax = after_stat(ymax)), geom="errorbar", width=.25,color="black") + 
...
my_comparisons <- list( c("IntraMethylated", "GTEx"), c("IntraUnmethylated", "GTEx"), 
                        c("IntraMethylated","IntraUnmethylated") )
p2<- ggplot(filter(mat_cdk, Category%in%c("IntraMethylated","IntraUnmethylated","GTEx")), 
            aes(x=Category,y=log2(value), fill=Type,color=Type))+
  scale_fill_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  scale_color_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  geom_boxplot(notch=FALSE,outlier.shape = NA, position = position_dodge(0.8))+
...
p3<- ggplot(filter(mat_cdk, Category%in%c("IntraAllelicMethylated","GTEx")), 
            aes(x=Category,y=log2(value), fill=Type,color=Type))+
  scale_fill_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  scale_color_manual(values = c("Tumour"="#8A7EC7","GTEx"="#69BDC3"))+
  geom_boxplot(notch=FALSE,outlier.shape = NA, position = position_dodge(0.8))+

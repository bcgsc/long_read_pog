...
in_file_processed2 <- pre_process_mavis_by_library(read.delim('combined2.tab'))
in_file_processed_tumour_comparison_copy <- pre_process_mavis_by_library(read.delim('combined2.tab'), FALSE)
in_file_processed_tumour_comparison <- pre_process_mavis_by_library(read.delim('combined2.tab'), FALSE)
in_file_processed_tumour_comparison_insertion <- pre_process_mavis_by_library_insertion(read.delim('combined2_ins.tab') %>% filter(event_type == 'insertion'), FALSE)
sample_sheet <- read.delim('long_pog_sample.tab')
oncoKB <- read.delim('cancerGeneList_210714.tsv')
```
...
in_file_processed2 <- in_file_processed2 %>% filter(replace_na(TRUE) & dgv == '') %>% filter(event_size>50|ins_size>50) %>% filter(break1_chromosome!='X'|break2_chromosome!='X') %>% filter(break1_chromosome!='Y'|break2_chromosome!='Y')
...
in_file_processed_tumour_comparison <- in_file_processed_tumour_comparison %>% filter(replace_na(TRUE) & dgv == '')  %>% filter(event_size>50|ins_size2>50) %>% filter(break1_chromosome!='X'|break2_chromosome!='X') %>% filter(break1_chromosome!='Y'|break2_chromosome!='Y')


sample_sheet <- sample_sheet %>%
                rename("POG_ID" = "library") %>%
                filter(Has_normal == 'Y') %>%
...
in_file_processed_tumour_comparison_insertion <- in_file_processed_tumour_comparison_insertion %>% filter(replace_na(TRUE) & dgv == '')  %>% filter(event_size>50) %>% filter(break1_chromosome!='X'|break2_chromosome!='X') %>% filter(break1_chromosome!='Y'|break2_chromosome!='Y') 
```

```{r dataset combination}
in_file_processed_tumour_comparison_no_ins <- in_file_processed_tumour_comparison %>% filter(event_type != 'insertion') %>% filter(break1_chromosome!='X'|break2_chromosome!='X') %>% filter(break1_chromosome!='Y'|break2_chromosome!='Y')
...
in_file_processed_tumour_comparison <- in_file_processed_tumour_comparison[, !(names(in_file_processed_tumour_comparison) %in% c("Anonymous_sample_ID_normal"))]
```
## Global SV Measures 
...
all_tumour_count <- read.delim('ready_for_graph1.tsv')
...
generate_density_main <- function(x, input_event_type, xlab_sv_type) {
  ...
  my.labels <- c('Illumina\n only','Illumina and\n Nanopore','Nanopore\n only')

  
  b <- x %>% filter(event_type==input_event_type) %>% ggplot(aes(x=Platform, fill=Platform)) + geom_bar() + theme_bw() + theme(legend.position="none") +
  scale_x_discrete(labels= my.labels) +
  xlab('') + 
  ylab(paste(xlab_sv_type,'\nCount',sep=' ')) 
  c <- plot_grid(b,a)
  return(c)
}
